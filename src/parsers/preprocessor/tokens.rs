...
pub enum ErrorKind {
    ...
    /// Returned when a user types a block comment on the same line as a preprocessor directive.
    /// Preprocessor directives can only contain line comments.
    /// Ex: `#define Foo /* comment */`
    IllegalBlockComment,
}
