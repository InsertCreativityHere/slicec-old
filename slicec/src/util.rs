...
pub struct SliceFile {
    ...
    pub filename: String,
    /// The path of the slice file, relative to where the slice compiler was run from
    /// (including it's '.ice' extension).
    pub relative_path: String,
    /// The raw text contained in the slice file.
    pub raw_text: String,
    /// The AST indices of all the top-level definitions in the slice file,
...
impl SliceFile {
    ...
    pub(crate) fn new(path: String, raw_text: String, contents: Vec<usize>, is_source: bool) -> Self {
        ...
        // If so, the '\n' block handles it, otherwise we store `index` (no plus one, because we've
        // already read ahead to the next character).
        ...
        SliceFile { filename, relative_path: path, raw_text, contents, is_source, line_positions }
    }
